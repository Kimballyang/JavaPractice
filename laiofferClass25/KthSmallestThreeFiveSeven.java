...
public class KthSmallestThreeFiveSeven {
	...
	public long kthII(int K) {
		...
		while (K > 1) {
			...
			//and 3^9a+1)*5^(b-1) < x, it means x*3 has to be already
			...
			if (three.peekFirst() < five.peekFirst() && three.peekFirst() <
					seven.peekFirst()) {
				result = three.pollFirst();
				three.offerLast(result * 3);
				five.offerLast(result * 5);
				seven.offerLast(result * 7);
			} else if (five.peekFirst() < three.peekFirst() && five.peekFirst() < 
					seven.peekFirst()) {
